...
const AddProduct = () => {
  ...
  const [selectedFiles, setSelectedFiles] = useState<File[]>([]); // 明确指定泛型参数
  const [previewImages, setPreviewImages] = useState<string[]>([]); // 假设这也是你的状态定义
  ...
  const onSubmit: OnSubmitType = async (data: FormValues) => {
    //儲存日期格式
